    ...
    \begin{column}{0.5\textwidth}
      \begin{figure}
        \begin{tikzpicture}
          ...
          \fill (0,0) circle (2pt) node[below] {C (h,k)};
          \fill (-2,0) circle (2pt) node[below] {F1};
          \fill (2,0) circle (2pt) node[below] {F2};
          \draw (-2,0) -- (2,0) node[midway,above] {a};
          \draw (0,-1) -- (0,1) node[midway,right] {b};
        \end{tikzpicture}
        \caption{Elipse y elementos}
      \end{figure}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
...
\begin{frame}{Parámetros de la Elipse}
  \frametitle{Parámetros de la Elipse}
  \begin{table}[]
    \begin{tabular}{|l|l|}
    \hline
    Ecuación General & $Ax^2 + Cy^2 + Dx + Ey + F = 0$ \\ 
    & $A = C, A\neq0$ \\ \hline
    Ecuación canónica & $\frac{(x-h)^2}{a^2} + \frac{(y-k)^2}{b^2} = 1$ \\ \hline
    Coordenadas del Centro & $(h, k)$ \\ \hline
    Coordenadas de los Focos & $(h \pm c, k)$, $c = \sqrt{a^2 - b^2}$ \\ \hline
    Ecuaciones de los ejes & Eje mayor: $y=k$, Eje menor: $x=h$ \\ \hline
    \end{tabular}
  \end{table}
\end{frame}
